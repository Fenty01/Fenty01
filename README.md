# Hi, I’m Iyanuoluwa 👋

**Aspiring Cloud & DevOps Engineer**

I’m on a journey to become an AWS Solutions Architect and DevOps professional. Currently training with **CoderTech Africa** and building hands-on projects to strengthen my cloud and automation skills.

-----

## 🎯 Currently Learning

![AWS](https://img.shields.io/badge/-AWS-232F3E?style=flat-square&logo=amazon-aws&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/-Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Jenkins](https://img.shields.io/badge/-Jenkins-D24939?style=flat-square&logo=jenkins&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)

-----

## 🛠️ Tools I’m Working With

- **Cloud:** AWS (EC2, S3, IAM, VPC)
- **Containers:** Docker, Docker Compose, Kubernetes
- **CI/CD:** Jenkins
- **Version Control:** Git & GitHub
- **OS:** Linux (Ubuntu)

-----

## 🚀 My Projects

### [DevOps](https://github.com/Fenty01/DEVOPS)

Practicing DevOps concepts — configurations, scripts, and automation workflows.

### [Multistage Realapp](https://github.com/Fenty01/multistage-realapp)

Learning Docker multi-stage builds for smaller, optimized container images.

### [Password Manager](https://github.com/Fenty01/Password-Manager)

A containerized multi-service application using Docker Compose — my hands-on project for learning container orchestration.

-----

## 🎓 Learning Goals for 2025

- [ ] Pass AWS Solutions Architect Associate exam
- [ ] Build and deploy projects on AWS
- [ ] Learn Terraform for Infrastructure as Code
- [ ] Set up CI/CD pipelines with Jenkins
- [ ] Deploy applications on Kubernetes

-----

## 📚 Currently Training

🎓 **CoderTech Africa** — AWS Solutions Architect & DevOps Program

-----

## 📫 Let’s Connect

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/your-linkedin)

-----

*Learning in public and documenting my cloud journey* ☁️
